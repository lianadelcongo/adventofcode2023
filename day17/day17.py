...
class State:
    def __init__(self, pos = (0,0), blocks = 3, direction = Direction.INITIAL):
    ...
    def possible_states(self)->"list[State]":
        ...
        if self.direction == Direction.INITIAL:
            return [State((1,0), 3, Direction.RIGHT), State((0,1), 3, Direction.DOWN)]
        if self.blocks > 1:
...
class Walk:
    def __init__(self, lines:list[str]):
        ...
        self.heat:defaultdict[tuple[int, int], int] = defaultdict(lambda: 10000000000000)
        self.limit = 0
        for y in range(0, len(lines)):
            for x in range(0, len(lines[0])):
                self.heat[(x,y)] = int(lines[y][x])
            self.limit += self.heat[(self.target_x, y)]
        self.limit *= 2

    def start(self):
        steps = {}
        steps[State()] = 0
        routes:list[tuple[State, int]] = [(State(), 0)]
        while len(routes) > 0:
            print(f"{len(steps)} {len(routes)}")
            (state, loss) = routes.pop(0)
            # print(f"Checking {state} {loss}")
            if state.pos[0] == self.target_x and state.pos[1] == self.target_y:
            ...
            for s in state.possible_states():
                ...
                if s in steps:
                    if steps[s] <= new_loss:
                        # print(f"Previously here {s} with {steps[str(s)]}")
                        continue
                steps[s] = new_loss
                routes.append((s, new_loss))
